...
Person::Person(char *name, Person* father, Person* mother){ //changed
    this->name = new char[strlen(name)+ 1]; //changed
...
Person::~Person(){
    delete[]children;
    delete[] name;          //program deleted children, but forgot name
}
...
char* Person::compute_relation(int level){
	...
	char *trash = temp;
        temp = temp2;
	delete[]trash; //no longer need temp2
    }
    return temp;
}
